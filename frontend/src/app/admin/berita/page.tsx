...
import { PlusIcon, TrashIcon, ArrowPathIcon, PencilSquareIcon, NewspaperIcon } from '@heroicons/react/24/outline';
    ...
    try {
      ...
      if (editingId) {
        ...
        const { error } = await supabase.from('articles').insert([{
          category: 'Berita Utama', title, slug, excerpt, content, cover_image: coverUrl!
        }]);
        if (error) {
    ...
    <div className="max-w-6xl mx-auto">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
        <div>
          <h1 className="text-3xl font-black tracking-tight text-slate-900 flex items-center gap-3">
            <NewspaperIcon className="w-8 h-8 text-primary" /> Berita Utama
          </h1>
          <p className="text-slate-500 font-medium mt-1">Kelola berita dan publikasi terbaru untuk ditampilkan di Beranda.</p>
        </div>
        <button 
          onClick={() => {
        ...
        >
          {isFormOpen ? 'Batal' : <><PlusIcon className="w-5 h-5" /> Tambah Berita</>}
        </button>
      </div>

      <Modal isOpen={isFormOpen} onClose={() => { setIsFormOpen(false); resetForm(); }} title={editingId ? 'Edit Berita Utama' : 'Tambah Berita Utama'} maxWidth="max-w-3xl">
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                <label className="block text-sm font-bold text-slate-700 mb-2">Judul</label>
                <input required type="text" value={title} onChange={e => setTitle(e.target.value)} placeholder="Contoh: Pemilihan Ketua OSIS" className="w-full border border-slate-300 rounded-lg px-4 py-2.5 outline-none focus:border-primary bg-white" />
              </div>
              <div>
                <label className="block text-sm font-bold text-slate-700 mb-2">Slug (URL)</label>
                <input required type="text" value={slug} onChange={e => setSlug(e.target.value)} placeholder="contoh-pemilihan-ketos" className="w-full border border-slate-300 rounded-lg px-4 py-2.5 outline-none focus:border-primary font-mono text-sm bg-white" />
              ...
              <div className="md:col-span-2">
                ...
                <textarea rows={8} value={content} onChange={e => setContent(e.target.value)} placeholder="Tuliskan berita lengkapnya di sini..." className="w-full border border-slate-300 rounded-lg px-4 py-2.5 outline-none focus:border-primary bg-white" />
              </div>
            </div>
            <div className="flex justify-end pt-4 border-t border-slate-200 mt-4">
              <button disabled={isSubmitting} type="submit" className="bg-primary text-white px-8 py-3 rounded-lg font-bold flex items-center gap-2 hover:bg-primary/90 disabled:opacity-50 disabled:cursor-not-allowed transition-all">
                {isSubmitting ? <><ArrowPathIcon className="w-5 h-5 animate-spin" /> Menyimpan...</> : (editingId ? 'Perbarui Berita' : 'Simpan Berita')}
              </button>
            </div>
          </form>
      </Modal>
      ...
      <div className="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden">
        {isLoading ? (
          ...
          <div className="p-12 text-center text-slate-500 font-medium">Belum ada berita yang ditambahkan.</div>
        ) : (
