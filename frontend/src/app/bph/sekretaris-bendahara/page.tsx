...
export default function SekretarisBendaharaPage() {
  const reduce = useReducedMotion();
  const [bphs, setBphs] = React.useState<any[]>([]);
  const [sekretarisPrograms, setSekretarisPrograms] = React.useState<any[]>([]);
  const [bendaharaPrograms, setBendaharaPrograms] = React.useState<any[]>([]);

  React.useEffect(() => {
    const fetchData = async () => {
      const [bphRes, prokerRes] = await Promise.all([
        supabase.from('bph').select('*').order('order_index', { ascending: true }),
        supabase.from('prokers').select('*').in('category', ['Sekretaris', 'Bendahara']).order('order_index', { ascending: true })
      ]);
      
      if (bphRes.data) setBphs(bphRes.data.filter(b => b.role.toLowerCase().includes('sekretaris') || b.role.toLowerCase().includes('bendahara')));
      if (prokerRes.data) {
        setSekretarisPrograms(prokerRes.data.filter(p => p.category === 'Sekretaris'));
        setBendaharaPrograms(prokerRes.data.filter(p => p.category === 'Bendahara'));
      }
    };
    fetchData();
  }, []);

  // Fetched with Promise.all in useEffect above
  ...
  return (
    <div className="flex flex-col min-h-dvh font-sans selection:bg-primary selection:text-white bg-background pt-24 lg:pt-32 pb-32">
      ...
      {/* PROGRAM KERJA SEKRETARIS (Table) */}
      <section className="container mx-auto px-6 lg:px-12 mb-24 lg:mb-32">
        <motion.div 
          initial={reduce ? false : { opacity: 0, y: 24 }}
          whileInView={{ opacity: 1, y: 0 }}
        ...
        >
          <div className="flex items-center gap-4 mb-8">
            ...
            <h2 className="text-3xl lg:text-4xl font-bold tracking-tighter text-foreground">Program Kerja: Sekretaris</h2>
          </div>
          ...
          <div className="flex md:hidden overflow-x-auto snap-x snap-mandatory gap-4 pb-8 -mx-6 px-6 hide-scrollbar" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
            {sekretarisPrograms.map((program, idx) => (
              <div key={program.id} className="snap-center shrink-0 w-[85vw] sm:w-[320px] border border-slate-200 rounded-3xl bg-white shadow-sm p-6 flex flex-col">
                <div className="flex justify-between items-start mb-4 gap-4">
                  <div className="flex-1">
                    <span className="text-slate-400 font-bold text-sm mb-1 block">#{idx + 1}</span>
      ...
      <section className="container mx-auto px-6 lg:px-12 mb-24 lg:mb-32">
        ...
        >
          <div className="flex items-center gap-4 mb-8">
            ...
            <h2 className="text-3xl lg:text-4xl font-bold tracking-tighter text-foreground">Program Kerja: Bendahara</h2>
          </div>
          
          {/* MOBILE CAROUSEL (Visible < md) */}
          <div className="flex md:hidden overflow-x-auto snap-x snap-mandatory gap-4 pb-8 -mx-6 px-6 hide-scrollbar" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
            {bendaharaPrograms.map((program, idx) => (
              <div key={program.id} className="snap-center shrink-0 w-[85vw] sm:w-[320px] border border-slate-200 rounded-3xl bg-white shadow-sm p-6 flex flex-col">
                <div className="flex justify-between items-start mb-4 gap-4">
                  <div className="flex-1">
                    <span className="text-slate-400 font-bold text-sm mb-1 block">#{idx + 1}</span>
